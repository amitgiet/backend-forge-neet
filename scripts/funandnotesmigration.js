/**
 * migrateNotesPdfsToDrive.js
 *
 * Fetches "notes" data from uploads/npcm_bio_database.db,
...
const LOG_FILE = path.join(LOG_DIR, 'notes_pdf_migration_log.json');

const FOLDER_NAME = 'notes_fun';
...
const uploadToDrive = async ({ fileName, buffer, contentType, folderId, accessToken }) => {
    ...
    // Axios doesn't handle multipart/related with raw buffers and boundary perfectly without some help or using FormData
    // But direct fetch or specific axios config works. Let's use Buffer.concat.
...
const getNotesFromDB = () => {
    return new Promise((resolve, reject) => {
        ...
        db.all("SELECT uniqueId, topicName, question FROM npcmBio WHERE quiztype = 'notes'", [], (err, rows) => {
            db.close();
            if (err) return reject(err);
            resolve(rows);
        });
...
const getSlug = (topicName) => {
    ...
    if (!first) return null; // guard empty slug from topics like ">> Something"
...
const buildPdfMetaUrl = (slug) =>
    `${FIREBASE_BUCKET_BASE}/notes2.0%2Fbiology%2F${encodeURIComponent(slug)}.pdf`;

const buildImageMetaUrl = (pageId) =>
    `${FIREBASE_BUCKET_BASE}/npcm%2FNOTES%2Flight%2F${encodeURIComponent(pageId)}.jpg`;
...
const main = async () => {
    ...
    // Map<uniqueId, entry> — prevents duplicates naturally, even under concurrent writes
    const logMap = new Map();
    for (const entry of loadExistingLogs()) {
        logMap.set(entry.uniqueId, entry);
    }

    // Async write mutex — serializes file writes so concurrent workers never interleave
    ...
    const processNote = async (note) => {
        ...
        try {
            ...
            console.error(`  PDF 404 for ${slug}, trying ${notePageIds.length} JPGs...`);
            ...
            try {
                ...
                slugToDriveResult[slug] = { mode: 'image_pages', pageCount: pageFiles.length, pageFiles };
                logMap.set(note.uniqueId, { uniqueId: note.uniqueId, slug, question: note.question, mode: 'image_pages', pageCount: pageFiles.length, pageFiles, pdfError: pdfError.message, status: 'success' });
                safeWrite();
                console.log(`  ✓ Fallback: ${slug} (${pageFiles.length} pages)`);
    ...
    // Wait for any queued writes, then do one final authoritative flush
    await writeLock;
    writeLogs([...logMap.values()]);
    ...
    console.log(`Log entries: ${logMap.size} (unique, no duplicates)`);
};
