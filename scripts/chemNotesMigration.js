/**
 * chemNotesMigration.js
 *
 * Fetches "notes" data from uploads/npcm_chem_database.db,
...
const DB_PATH = path.join(__dirname, '../uploads/npcm_chem_database.db');
const LOG_DIR = path.join(__dirname, 'logs');
const LOG_FILE = path.join(LOG_DIR, 'chem_notes_pdf_migration_log.json');

const FOLDER_NAME = 'chem_notes_fun';
...
const getNotesFromDB = () => {
    return new Promise((resolve, reject) => {
        ...
        // Chem DB table is npcmChem and column is quizType
        db.all("SELECT uniqueId, topicName, question FROM npcmChem WHERE quizType = 'notes'", [], (err, rows) => {
            db.close();
            if (err) return reject(err);
            resolve(rows);
        });
...
const buildPdfMetaUrl = (slug) =>
    `${FIREBASE_BUCKET_BASE}/notes2.0%2Fchemistry%2F${encodeURIComponent(slug)}.pdf`;

// Updated pattern for Chemistry images: npcm%2FChemistry%2FNotes%2Flight%2F${pageId}.jpg
const buildImageMetaUrl = (pageId) =>
    `${FIREBASE_BUCKET_BASE}/npcm%2FChemistry%2FNotes%2Flight%2F${encodeURIComponent(pageId)}.jpg`;
