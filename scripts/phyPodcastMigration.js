/**
 * phyPodcastMigration.js
 *
 * Migrates Physics podcast audio files from Firebase to Google Drive.
 * Fetches IDs from the `question` field in npcm_phy_database.db.
 */
...
const LOG_FILE = path.join(LOG_DIR, 'phy_podcast_migration_log.json');

const FOLDER_NAME = 'phy_podcasts_fun';
...
const getPodcastsFromDB = () => {
    return new Promise((resolve, reject) => {
        ...
        db.all("SELECT uniqueId, topicName, question FROM npcmPhy WHERE quizType = 'podcast'", [], (err, rows) => {
            db.close();
            if (err) return reject(err);
            resolve(rows);
        });
...
const buildAudioMetaUrl = (id) =>
    `${FIREBASE_BUCKET_BASE}/npcm%2Fphysics%2FPodcast%2F${encodeURIComponent(id)}.mp3`;
