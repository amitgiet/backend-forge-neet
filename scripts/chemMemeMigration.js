/**
 * chemMemeMigration.js
 *
 * Migrates Chemistry meme images from Firebase to Google Drive.
 * Handles comma-separated IDs in the `question` field in npcm_chem_database.db.
 */
...
const LOG_FILE = path.join(LOG_DIR, 'chem_meme_migration_log.json');

const FOLDER_NAME = 'chem_memes_fun';
...
const getMemesFromDB = () => {
    return new Promise((resolve, reject) => {
        ...
        db.all("SELECT uniqueId, topicName, question FROM npcmChem WHERE quizType COLLATE NOCASE = 'meme'", [], (err, rows) => {
            db.close();
            if (err) return reject(err);
            resolve(rows);
        });
...
const buildMemeMetaUrl = (id) =>
    `${FIREBASE_BUCKET_BASE}/npcm%2FChemistry%2Fmeme%2F${encodeURIComponent(id)}.jpg`;
