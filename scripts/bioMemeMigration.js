 ...
 * bioMemeMigration.js
 *
 * Migrates Biology meme images from Firebase to Google Drive.
 * Handles comma-separated IDs in the `question` field in npcm_bio_database.db.
 */
...
const LOG_FILE = path.join(LOG_DIR, 'bio_meme_migration_log.json');

const FOLDER_NAME = 'bio_memes_fun';
...
const getMemesFromDB = () => {
    return new Promise((resolve, reject) => {
        ...
        db.all("SELECT uniqueId, topicName, question FROM npcmBio WHERE quizType = 'meme'", [], (err, rows) => {
            db.close();
            if (err) return reject(err);
            resolve(rows);
        });
...
const buildMemeMetaUrl = (id) =>
    `${FIREBASE_BUCKET_BASE}/npcm%2Fmeme%2F${encodeURIComponent(id)}.jpg`;
    ...
    const processMemeRow = async (row) => {
        ...
        for (const memeId of rawIds) {
            try {
                ...
                driveFiles.push({
                    pageId: memeId, // Reusing pageId field as imageId
