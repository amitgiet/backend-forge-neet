/**
 * chemPodcastMigration.js
 *
 * Migrates Chemistry podcast audio files from Firebase to Google Drive.
 * Fetches IDs from the `question` field in npcm_chem_database.db.
 */
...
const LOG_FILE = path.join(LOG_DIR, 'chem_podcast_migration_log.json');

const FOLDER_NAME = 'chem_podcasts_fun';
...
const getPodcastsFromDB = () => {
    return new Promise((resolve, reject) => {
        ...
        db.all("SELECT uniqueId, topicName, question FROM npcmChem WHERE quizType = 'podcast'", [], (err, rows) => {
            db.close();
            if (err) return reject(err);
            resolve(rows);
        });
...
const buildAudioMetaUrl = (id) =>
    `${FIREBASE_BUCKET_BASE}/npcm%2FChemistry%2Fpodcast%2F${encodeURIComponent(id)}.mp3`;
...
const main = async () => {
    ...
    const processPodcast = async (p) => {
        ...
        const audioId = String(p.question || '').trim();
        if (!audioId) {
